...
single_sections13 = {key: value for key, value in dict_of_sections1.items() if len(value) == 1}
sections_to_summarize13 = {key: value for key, value in dict_of_sections1.items() if len(value) > 1}
...
master_one_at_a_time1 = {}

#DOCUMENTATION ALERT: count rounds
count_one_at_a_time1 = 0
...
for key, values in sections_to_summarize13.items():
    source_of_first = values[0]['source']
    topic_of_first = values[0]['topic']
    clauses_of_first = sections_and_clauses_doc[source_of_first][topic_of_first] #Collect clauses of first entry to represent the base
    base_parts1[key] = clauses_of_first
    # Add the rest of the values to rest_parts (will be compared to base later in LLM loop)
    rest_parts1[key] = values[1:]



#Iterating over two sections at a time that have multiple appearances (to make Master list of clauses)
for main_key, values in rest_parts1.items():
    current_base = base_parts1[main_key]
    topics = [main_key] #storing synonyms of that topic used in different documents
    for value in values:
        count_one_at_a_time1 += 1
        source = value['source']
        topic = value['topic']
        topics.append(topic)
        if source in sections_and_clauses_doc: #Use NDA file name to access its sections and clauses
            if topic in sections_and_clauses_doc[source]: #Use topic to access its clauses
                next_part = sections_and_clauses_doc[source][topic] #Collect clauses to the list
            else:
                print(f"Topic '{topic}' not found in {source}")
        else:
            print(f"Source '{source}' not found in sections_and_clauses_doc")
    ...
    if section_title in master_one_at_a_time1: #Error check, if master list already has this section title
...
unique_sections_single_1 = {} #Will be added to Master as is
sections_to_be_merged_single_1 = {} #Will be compared and possibly merged with existing sections
...
for key, values in single_sections13.items():
    for value in values:
        source = value['source']
        topic = value['topic']
        ...
        List of existing sections and their clauses: {master_one_at_a_time1} 
        ...
        if potential_section == "New section":
            ...
            unique_sections_single_1[topic] = clauses
        else:
            print(f"The section {topic} is similar to {potential_section}.")
            print(f"Its clauses are: {clauses}")
            print(f"The existing section: {master_one_at_a_time1[potential_section]}  \n \n")
            sections_to_be_merged_single_1[potential_section] = {'topic': topic, 'clauses': clauses}
...
master_one_at_a_time_copy1 = master_one_at_a_time1.copy()


for topic, clauses in unique_sections_single_1.items():
    print(f"{topic}: {clauses} \n")
    if topic in master_one_at_a_time_copy1:
        print(f"Error: Topic '{topic}' already exists in the Master list.")


# Structure of code for adding
# master_one_at_a_time_copy1['topic'] = unique_sections_single_1['topic']
master_one_at_a_time_copy1['Recitals'] = unique_sections_single_1['Recitals']
master_one_at_a_time_copy1['Right to Disclose'] = unique_sections_single_1['Right to Disclose']


################            Merging rest of the sections to Master list


for key, value in sections_to_be_merged_single_1.items():
    print(f"Section for potential merging: {value} \n")
    print(f"It can be merged with {key}: {master_one_at_a_time1[key]} \n")
...
sections_to_update13 = ['No License Granted, No Transfer of Rights, Title, or Interest', 'Preamble']

for key in sections_to_update13:
    topic_to_update = sections_to_be_merged_single_1[key]['topic']
    clauses_to_update = sections_to_be_merged_single_1[key]['clauses']
    print(f"Updating '{key}' with the topic '{topic_to_update}' and clauses: {clauses_to_update}")
    print(f"Current content: {master_one_at_a_time_copy1[key]} \n")

    text_to_update = f"""
    First set of clauses: {master_one_at_a_time_copy1[key]}
    ...
    for llm_key, value in output.items(): #Adding to master dict
        if len(identified_topics) == 1: #All good, only one section identified
            master_one_at_a_time_copy1[key] = value
            print(f"Final output for {key}: {master_one_at_a_time_copy1[key]} \n")
        else:
...
for key, value_list in master_one_at_a_time_copy1.items():
    master_one_at_a_time_copy1[key] = [f"{i+1}. {item}" for i, item in enumerate(value_list)]


# New dictionary to store counts of clauses
clauses_count_one_at_a_time_1 = {}
for key, value in master_one_at_a_time_copy1.items():
    clauses_count_one_at_a_time_1[key] = [{"clause": clause, "count": 0} for clause in value]
...
for document in file_paths:
    ...
    for topic, clauses in master_one_at_a_time_copy1.items():
        print(f"Topic: {topic}")
        ...
        while True:
            ...
            if len(clauses) == len(output[output_key]): #If all clauses have Boolean value, break the loop
                comparison_results = []
                if output_key in output and topic in clauses_count_one_at_a_time_1:
                    for i, (item1, item2) in enumerate(zip(output[output_key], clauses_count_one_at_a_time_1[topic]), start=1):
...
for key, value in clauses_count_one_at_a_time_1.items():
    print(f"Topic: {key}")
    for item in value:
...
with open(os.path.join(folder_path, 'master_one_at_a_time_copy1.json'), 'w') as f:
    json.dump(master_one_at_a_time_copy1, f)

with open(os.path.join(folder_path, 'clauses_count_one_at_a_time_1.json'), 'w') as f:
    json.dump(clauses_count_one_at_a_time_1, f)


# Think about output style!


with open(os.path.join(folder_path, '13_Entire_One_section.txt'), 'w') as f:
    for key, value in clauses_count_one_at_a_time_1.items():
        f.write(f"Topic: {key} \n\n")
        for item in value:
...
with open(os.path.join(folder_path, 'Execution_counter.txt'), 'a') as f:
    f.write(f"13_Entire One section \n")
    f.write("\n")
    f.write(f"To summarize entire one section (comparing one section at a time), it took: {count_one_at_a_time1} runs.\n")
    f.write("\n")
    f.write("\n")
    f.write("\n")
