...
example_sections1 = sections_and_clauses_doc['NDA1'].copy() #This will contain example of each separate section
dict_of_sections1 = {key : [{'topic' : key, 'source': 'NDA1'}] for key in sections_and_clauses_doc['NDA1'].keys()} #This dict is a list of similar sections, with source NDA for each section title (that will be merged later)
...
for file_name, file_content in sections_without_nda1_1.items():
    for section, clauses in file_content.items():
        
        example_data_sections = f""" 
        List of existing sections and their clauses: {example_sections1}
        ...
        if potential_section == "New section":
            if section in dict_of_sections1: #To avoid updating keys, check if section already exists in dict
                print(f"Title '{section}' was in dict, but added as a new section.")
                print(f"Added as \"{section} ({file_name})\" \n")
                dict_of_sections1[f"{section} ({file_name})"] = [{'topic' : section, 'source': file_name}] #Have to name key differently
                example_sections1[f"{section} ({file_name})"] = clauses
            else: #This is the normal case, that the section is completely new for dict
                dict_of_sections1[section] = [{'topic' : section, 'source': file_name}]
                example_sections1[section] = clauses
        elif potential_section in dict_of_sections1: #If the section is already in dict, append the new section to it as a collection of synonyms
            dict_of_sections1[potential_section].append({'topic': section, 'source': file_name})
        else:
            print(f"Something went wrong, section was {section} from file {file_name}, and output was {potential_section} \n")
            
# Check the reslult
for key, value in dict_of_sections1.items():
    print(f"{key}")
    for i in value:
        print(f"    {i}")
...
single_sections12 = {key: value for key, value in dict_of_sections1.items() if len(value) == 1}
sections_to_summarize12 = {key: value for key, value in dict_of_sections1.items() if len(value) > 1}
...
#DOCUMENTATION ALERT: count of all sections at once
count_all_sections_at_once1 = 0

#Iterating over the sections that have multiple appearances (to make Master list of clauses)
for main_key, values in sections_to_summarize12.items(): 
    count_all_sections_at_once1 += 1
    ...
    for value in values:
        ...
        topic = value['topic']
        topics.append(topic)
        if source in sections_and_clauses_doc: #Use NDA file name to access its sections and clauses
            if topic in sections_and_clauses_doc[source]: #Use topic to access its clauses
                clauses.append(sections_and_clauses_doc[source][topic]) #Collect clauses to the list
            else:
    ...
    if section_title in master_all_at_once1: #Error check, if master list already has this section title
...
unique_sections_all_1 = {} #Will be added to Master as is
sections_to_be_merged_all_1 = {} #Will be compared and possibly merged with existing sections
...
for key, values in single_sections12.items():
    for value in values:
        source = value['source']
        topic = value['topic']
        clauses = sections_and_clauses_doc[source][topic]

        example_data_all = f""" 
        List of existing sections and their clauses: {master_all_at_once1} 
        ...
        if potential_section == "New section":
            ...
            unique_sections_all_1[topic] = clauses
        else:
            print(f"The section {topic} is similar to {potential_section}.")
            print(f"Its clauses are: {clauses}")
            print(f"The existing section: {master_all_at_once1[potential_section]}  \n \n")
            sections_to_be_merged_all_1[potential_section] = {'topic': topic, 'clauses': clauses}
...
master_all_at_once1_copy1 = master_all_at_once1.copy()

#Adding unique sections that are worth adding
for topic, clauses in unique_sections_all_1.items():
    print(f"{topic}: {clauses} \n")
    if topic in master_all_at_once1_copy1:
        print(f"Error: Topic '{topic}' already exists in the Master list.")
        


# Structure of code for adding
# master_all_at_once1_copy1['topic'] = unique_sections_all_1['topic']
master_all_at_once1_copy1['Recitals'] = unique_sections_all_1['Recitals']
master_all_at_once1_copy1['Right to Disclose'] = unique_sections_all_1['Right to Disclose']
...
### Attention! This might contradict a little, because here we are going through one section at a time -> should I not do this?

for key, value in sections_to_be_merged_all_1.items():
    print(f"Section for potential merging: {value} \n")
    print(f"It can be merged with {key}: {master_all_at_once1[key]} \n")
...
sections_to_update12 = ['No License Granted, No Transfer of Rights, Title or Interest, and Acknowledgments', 'Preamble']


for key in sections_to_update12:
    topic_to_update = sections_to_be_merged_all_1[key]['topic']
    clauses_to_update = sections_to_be_merged_all_1[key]['clauses']
    print(f"Updating '{key}' with the topic '{topic_to_update}' and clauses: {clauses_to_update}")
    print(f"Current content: {master_all_at_once1_copy1[key]} \n")

    text_to_update = f"""
    First set of clauses: {master_all_at_once1_copy1[key]}
    ...
    for llm_key, value in output.items(): #Adding to master dict
        if len(identified_topics) == 1: #All good, only one section identified
            master_all_at_once1_copy1[key] = value
            print(f"Final output for {key}: {master_all_at_once1_copy1[key]} \n")
        else:
...
for key, value_list in master_all_at_once1_copy1.items():
    master_all_at_once1_copy1[key] = [f"{i+1}. {item}" for i, item in enumerate(value_list)]


# New dictionary to store counts of clauses
clauses_count_all_at_once_1 = {}
for key, value in master_all_at_once1_copy1.items():
    clauses_count_all_at_once_1[key] = [{"clause": clause, "count": 0} for clause in value]
...
for document in file_paths:
    ...
    for topic, clauses in master_all_at_once1_copy1.items():
        print(f"Topic: {topic}")
        ...
        while True:
            ...
            if len(clauses) == len(output[output_key]): #If all clauses have Boolean value, break the loop
                comparison_results = []
                if output_key in output and topic in clauses_count_all_at_once_1:
                    for i, (item1, item2) in enumerate(zip(output[output_key], clauses_count_all_at_once_1[topic]), start=1):
...
for key, value in clauses_count_all_at_once_1.items():
    print(f"\n Topic: {key} \n")
    for item in value:
...
with open(os.path.join(folder_path, 'dict_of_sections1.json'), 'w') as f:
    json.dump(dict_of_sections1, f)


with open(os.path.join(folder_path, 'master_all_at_once1_copy1.json'), 'w') as f:
    json.dump(master_all_at_once1_copy1, f)


with open(os.path.join(folder_path, 'clauses_count_all_at_once_1.json'), 'w') as f:
    json.dump(clauses_count_all_at_once_1, f)
...
with open(os.path.join(folder_path, '12_Entire_All_sections.txt'), 'w') as f:
    for key, value in clauses_count_all_at_once_1.items():
        f.write(f"Topic: {key} \n\n")
        for item in value:
...
with open(os.path.join(folder_path, 'Execution_counter.txt'), 'a') as f:
    f.write(f"12_Entire All sections \n")
    f.write("\n")
    f.write(f"To summarize entire all sections (comparing all sections of the same topic at once), it took: {count_all_sections_at_once1} runs.\n")
    f.write("\n")
    f.write("\n")
    f.write("\n")


with open(os.path.join(folder_path, 'dict_of_synonym_sections1.txt'), 'w') as f:
    for key, value in dict_of_sections1.items():
        f.write(f"{key}: \n\n")
        for item in value:
            f.write(f"    {item} \n")
